...
void rvizCallback(const geometry_msgs::PoseStampedConstPtr &)
{
    for (int num = 0; num < 1; num++)
    {
        ...
        waypoints.push_back(Eigen::Vector3d(-11.067, -11.0858, H));
        waypoints.push_back(Eigen::Vector3d(L, -L, H));
        // waypoints.push_back(Eigen::Vector3d(L, L, H));
...
int main(int argc, char **argv)
{
    ...
    // 初始化
    std::string filename = "/home/ly/ws_sfc/src/sfc/sfc_planner/config/2d.yaml";
    ...
    while (ros::ok())
    {
        ...
        sfcenv_->publishDist(frame, dist_pub_, height = 0.9);

        sfcenv_->publishGridDist(frame, grid_dist_pub_, height = 0.9);
        sfcenv_->publishKDDist(frame, POI_dist_pub_, height = 0.9);
