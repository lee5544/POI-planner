...
private:
    // 工作空间
    SFCEnv::Ptr sfcenv_ptr_;    // 基本的栅格地图
    POIContainer interest_obs_; // 优化过程中发现（关注）的障碍物
    bool refineSFCEnv(Eigen::MatrixXd traj);
...
public:
    void init(ros::NodeHandle nh, std::string filename, SFCEnv::Ptr sfcenv, bool verbose = false);

    // 设置输入
    void setEnv(SFCEnv::Ptr sfcenv);
    void setOptVar(Eigen::MatrixXd opt_var);
    void setPOIs(POIContainer interest_obs); // 输入搜索过程中的兴趣障碍物
    ...
    // 优化主程序
    bool optimize(bool change = true);
    ...
    typedef std::shared_ptr<SFCNlopt> Ptr;
