...
// Optimized setObstacles for SOGMMap.cpp
void SOGMMap::setObstacles(pcl::PointCloud<pcl::PointXYZ> *ptws_hit_ptr)
{
    // Prepare for updated occupancy indices
    new_occ_.clear();
    new_occ_.reserve(ptws_hit_ptr->size());

    const double thresh = min_occupancy_log_;
    const double max_log = clamp_max_log_;

    for (const auto& pt : ptws_hit_ptr->points)
    {
        // WorldToVoxel and boundary check
        Eigen::Vector3i voxel = WorldToVoxel(Eigen::Vector3d(pt.x, pt.y, pt.z));
        if (!isInMap(voxel))
            continue;

        int idx = VoxelToIndex(voxel);
        double& cell = occ_value_[idx];

        // Only update if not already occupied
        if (cell < thresh)
        {
            cell = max_log;
            new_occ_.push_back(idx);
        }
        else
        {
            // Already occupied: just ensure max log remains
            cell = max_log;
        }
    }
}
